...
В рамках тестового задания нужно было реализовать API для управления задачами со следующими требованиями:

- реализовать CRUD для сущности `TaskItem`
- использовать `ASP.NET Core Minimal API`
- хранить данные в `PostgreSQL` через `EF Core`
- при завершении задачи публиковать событие в `RabbitMQ`
- если `RabbitMQ` недоступен, завершение задачи должно оставаться успешным, а ошибка публикации должна попадать в лог
- при одновременном завершении одной и той же задачи один запрос должен завершаться успешно, а второй возвращать `409 Conflict`
- добавить интеграционные тесты на ключевые сценарии
